...
parameters {
  ...
  vector[N] alpha_pr;
  //vector[N] beta_pr; //seems to just initialize the prior, probably no change needed
  vector[N] delta_pr;
  vector[N] tau_pr;
}
transformed parameters {
  ...
  //vector<lower=.5,upper=.5>[N] beta;  # initial bias //DAW: I changed this to .5
...
//    beta[i] = Phi_approx( mu_p[2] + sigma[2] * beta_pr[i] );
    tau[i]  = Phi_approx( mu_p[3] + sigma[3] * tau_pr[i] ) * (minRT[N]-RTbound) + RTbound;
  }
  alpha = exp( mu_p[1] + sigma[1] * alpha_pr );
  delta = exp( mu_p[2] + sigma[2] * delta_pr );
}
model {
  # Hyperparameters
  mu_p  ~ normal(0,1);        
  ...
  alpha_pr ~ normal(0, 1);
 // beta_pr  ~ normal(0, 1); //sets the prior for individual level parameters
  delta_pr ~ normal(0, 1);
  tau_pr   ~ normal(0, 1);
  ...
  for (i in 1:N) {
    ...
    RTu[i,:Nu[i]] ~ wiener(alpha[i], tau[i], .5, delta[i]);
    RTl[i,:Nl[i]] ~ wiener(alpha[i], tau[i], 1-.5, -delta[i]);
...
generated quantities {
 ...
 // real<lower=.5,upper=.5> mu_beta;  # initial bias // DAW: changed to .5 and .5
  ...
  mu_delta = exp(mu_p[2]);
  mu_tau   = Phi_approx(mu_p[3]) * (mean(minRT)-RTbound) + RTbound;

  { # local section, this saves time and space
    # Begin subject loop
    for (i in 1:N) {
      log_lik[i] = wiener_lpdf(RTu[i,:Nu[i]] | alpha[i], tau[i], .5, delta[i]);
      log_lik[i] = log_lik[i] + wiener_lpdf(RTl[i,:Nl[i]] | alpha[i], tau[i], 1-.5, -delta[i]);
    }
  }
}
