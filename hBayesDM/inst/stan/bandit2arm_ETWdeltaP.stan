...
transformed data {
  ...
  initET = rep_vector(0.1, 2); //starting for ET traces is zero
  //vector[2] initet;  # initial values for EV
  //initet <- [0 0];
  //vector[2] initet;  # initial values for EV
...
model {
  ...
  for (i in 1:N) {
    ...
    for (t in 1:(Tsubj[i])) {        
      ...
      // update choice for the chosen option
      ev[choice[i,t]] = ev[choice[i,t]] + A[i] * (outcome[i,t] - ev[choice[i,t]]); //reward           based on credit
      
      
      for (j in 1:2) {
      # value updating (learning)
      normet[j]=et[j]/sumet;
      etv[j] = etv[j] + et[j]*(outcome[i,t] - ev[j]); //reward based on credit
