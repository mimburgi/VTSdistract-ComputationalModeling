...
model {
  ...
  for (i in 1:N) {
    ...
    for (t in 1:(Tsubj[i])) {        
      ...
      sumet = exp(et[1])+exp(et[2]);
      
      for (j in 1:2) {
      # value updating (learning)
      normet[j]=exp(et[j])/sumet;
      iv[j] = iv[j] + normet[j]*(outcome[i,t] - ev[j]); //reward based on credit
