...
model {
  ...
  for (i in 1:N) {
    ...
    for (t in 1:(Tsubj[i])) {        
      ...
      # increment average reward learner 
      et[choice[i,t]] = et[choice[i,t]] + A[i]*(outcome[i,t]-AvgR);
      AvgR = AvgR + A[i]*(outcome[i,t]-et[choice[i,t]]);
      ev[choice[i,t]] = ev[choice[i,t]] + A[i]*(outcome[i,t]-ev[choice[i,t]]);
