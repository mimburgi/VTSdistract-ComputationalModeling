#' baseline model for RelationalRL
#' 
#' @description 
...
#' \describe{
#'  \item{\code{model}}{Character string with the name of the model (\code{"baseline3arm"}).}
...
#' output <- baseline3arm(data = "example", niter = 2000, nwarmup = 1000, nchain = 3, ncore = 3)
#' 
...
baseline3arm <- function(data          = "choose",
                             niter         = 3000, 
                             nwarmup       = 1000, 
                             nchain        = 4,
                             ncore         = 1, 
  ...
  if (modelRegressor) { # model regressors (for model-based neuroimaging, etc.)
    ...
    modelPath <- system.file("stan", "baseline3arm.stan", package="hBayesDM")  
  }
