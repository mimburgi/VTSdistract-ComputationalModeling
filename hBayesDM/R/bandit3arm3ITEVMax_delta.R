...
#' \describe{
#'  \item{\code{model}}{Character string with the name of the model (\code{"bandit3arm3ITEVMax_delta"}).}
...
#' output <- bandit3arm3ITEVMax_delta(data = "example", niter = 2000, nwarmup = 1000, nchain = 3, ncore = 3)
#' 
...
bandit3arm3ITEVMax_delta <- function(data          = "choose",
                             niter         = 3000, 
                             nwarmup       = 1000, 
                             nchain        = 4,
                             ncore         = 1, 
  ...
  if (modelRegressor) { # model regressors (for model-based neuroimaging, etc.)
    ...
    modelPath <- system.file("stan", "bandit3arm3ITEVMax_delta.stan", package="hBayesDM")  
  }
  ...
  modelName <- "bandit3arm3ITEVMax_delta"
