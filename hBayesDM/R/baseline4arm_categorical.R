#' logistic regression model with just choices as predictors 
#' 
#' @description 
...
#' output <- baseline4arm_categorical(data = "example", niter = 2000, nwarmup = 1000, nchain = 3, ncore = 3)
#' 
...
baseline4arm_categorical <- function(data          = "choose",
                              niter         = 3000, 
                              nwarmup       = 1000,
                              nchain        = 4,
                              ncore         = 1, 
  ...
  if (modelRegressor) { # model regressors (for model-based neuroimaging, etc.)
    ...
    modelPath <- system.file("stan", "igt_vpp.stan", package="hBayesDM")
  }
  ...
  numPars <- 4 
  POI     <- c("mu_C1", "mu_C2", "mu_C3", "mu_C4", 
               "sigma", 
               "C1", "C2", "C3", "C4",
               "log_lik")
  
  modelName <- "baseline4arm_categorical"
  ...
  if (inits[1] != "random") {
    ...
    genInitList <- function() {
      list(
        mu_p      = c( inits_fixed[1], inits_fixed[2], inits_fixed[3], inits_fixed[4]  ),
        sigma     = c(1.0, 1.0, 1.0, 1.0),
        C1_pr    = rep( inits_fixed[1], numSubjs),
        C2_pr    = rep( inits_fixed[2], numSubjs),
        C3_pr    = rep( inits_fixed[3], numSubjs),
        C4_pr    = rep( inits_fixed[4], numSubjs),        
      )
    }
  } else {
    genInitList <- "random"
  ...
  for (i in 1:numSubjs) {
    if (indPars=="mean") {
      allIndPars[i, ] <- c( mean(C1[, i]), 
                            mean(C2[, i]), 
                            mean(C3[, i]), 
                            mean(C4[, i]) )
    } else if (indPars=="median") {
      allIndPars[i, ] <- c( median(C1[, i]), 
                            median(C2[, i]), 
                            median(C3[, i]), 
                            median(C4[, i]) )
    } else if (indPars=="mode") {
      allIndPars[i, ] <- c( estimate_mode(C1[, i]), 
                            estimate_mode(C2[, i]), 
                            estimate_mode(C3[, i]), 
                            estimate_mode(C4[, i]) )
    }
