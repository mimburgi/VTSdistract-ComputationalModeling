...
#' \describe{
#'  \item{\code{model}}{Character string with the name of the model (\code{"dd_cs"}).}
...
#' 
#' @references 
#' Ebert, J. E. J., & Prelec, D. (2007). The Fragility of Time: Time-Insensitivity and Valuation of the Near and Far Future. 
#' Management Science. http://doi.org/10.1287/mnsc.1060.0671
#' 
...
dd_cs <- function(data          = "choose",
  ...
  if (inits[1] != "random") {
    ...
    genInitList <- function() {
      list(
        mu_p    = c( qnorm(inits_fixed[1]), qnorm(inits_fixed[2] / 10), qnorm(inits_fixed[3] / 5) ), 
        sigma   = c(1.0, 1.0, 1.0), 
