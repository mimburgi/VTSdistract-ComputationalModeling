...
prl_fictitious_multipleB <- function(data           = "choice",
                                     niter          = 3000, 
                                     nwarmup        = 1000, 
                                     nchain         = 1,
                                     ncore          = 1, 
  ...
  if (modelRegressor) { # model regressors (for model-based neuroimaging, etc.)
    ...
    modelPath <- system.file("stan", "prl_fictitious_multipleB.stan", package="hBayesDM")
  }
  ...
  if (inits[1] != "random") {
    ...
    genInitList <- function() {
      list(
        mu_eta_pr   = qnorm(inits_fixed[1]),
        mu_alpha_pr = qnorm(inits_fixed[2]),
        mu_beta_pr  = qnorm(inits_fixed[3] / 10),
        sd_eta      = 1,
        sd_alpha    = 1,
        sd_beta     = 1,
