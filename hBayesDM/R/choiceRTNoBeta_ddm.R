...
choiceRTNoBeta_ddm <- function(data           = "choose",
                         niter          = 3000, 
                         nwarmup        = 1000, 
                         nchain         = 4,
                         ncore          = 1, 
  ...
  if (modelRegressor) { # model regressors (for model-based neuroimaging, etc.)
    ...
    modelPath <- system.file("stan", "choiceRTNoBeta_ddm.stan", package="hBayesDM")
  }
  ...
  if (inits[1] != "random") {
    ...
    genInitList <- function() {
      list(
        ...
        delta_pr = rep( log(inits_fixed[2]), numSubjs),
        tau_pr   = rep( qnorm(inits_fixed[3]), numSubjs)
      )
    }
  } else {
    genInitList <- "random"
