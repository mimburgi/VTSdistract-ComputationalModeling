#' Two-Arm Bandit Task ET model nested within bandit model with perseveration
#' 
#' @description 
...
#' 
#' \strong{MODEL:}
#' Rescorla-Wagner (delta) model with Eligibility traces (Sutton & Barto 1998) and perseveration
#' 
...
#' \describe{
#'  \item{\code{model}}{Character string with the name of the model (\code{"bandit2arm_ETNestP"}).}
...
#' output <- bandit2arm_ETNestP(data = "example", niter = 2000, nwarmup = 1000, nchain = 3, ncore = 3)
#' 
...
bandit2arm_ETNestP <- function(data          = "choose",
                             niter         = 3000, 
                             nwarmup       = 1000, 
                             nchain        = 4,
                             ncore         = 1, 
  ...
  if (modelRegressor) { # model regressors (for model-based neuroimaging, etc.)
    ...
    modelPath <- system.file("stan", "bandit2arm_ETNestP.stan", package="hBayesDM")  
  }
  ...
  numPars <- 4
  POI     <- c("mu_A", "mu_tau", "mu_gam", "mu_P",
               "sigma",
               "A", "tau", "gam", "P",
               "log_lik")
  
  modelName <- "bandit2arm_ETNestP"
  ...
  if (inits[1] != "random") {
    ...
    genInitList <- function() {
      list(
        ...
        gam_pr   = rep(qnorm(inits_fixed[3]), numSubjs),
      )
    }
  } else {
    genInitList <- "random"
