#' Two-Arm Bandit Task with eligibility traces instead of delta param
#' 
#' @description 
#' Hierarchical Bayesian Modeling of the Two-Arm Bandit Task where "A" (learning rate) is now an eligibility trace decay, "tau" (inverse temperature).
#' 
#' \strong{MODEL:}
#' Rescorla-Wagner (delta) model replaced by eligibility traces 
#' 
...
#' \describe{
#'  \item{\code{model}}{Character string with the name of the model (\code{"bandit2arm_ETonlydelta"}).}
...
#' output <- bandit2arm_ETonlydelta(data = "example", niter = 2000, nwarmup = 1000, nchain = 3, ncore = 3)
#' 
...
bandit2arm_ETonlydelta <- function(data          = "choose",
                             niter         = 3000, 
                             nwarmup       = 1000, 
                             nchain        = 4,
                             ncore         = 1, 
  ...
  if (modelRegressor) { # model regressors (for model-based neuroimaging, etc.)
    ...
    modelPath <- system.file("stan", "bandit2arm_ETonlydelta.stan", package="hBayesDM")  
  }
  ...
  modelName <- "bandit2arm_ETonlydelta"
