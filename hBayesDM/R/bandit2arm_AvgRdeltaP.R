#' Two-Arm Bandit Task weighted model with perseveration
#' 
#' @description 
...
#' \describe{
#'  \item{\code{model}}{Character string with the name of the model (\code{"bandit2arm_AvgRdeltaP"}).}
...
#' output <- bandit2arm_AvgRdeltaP(data = "example", niter = 2000, nwarmup = 1000, nchain = 3, ncore = 3)
#' 
...
bandit2arm_AvgRdeltaP <- function(data          = "choose",
                             niter         = 3000, 
                             nwarmup       = 1000, 
                             nchain        = 4,
                             ncore         = 1, 
  ...
  if (modelRegressor) { # model regressors (for model-based neuroimaging, etc.)
    ...
    modelPath <- system.file("stan", "bandit2arm_AvgRdeltaP.stan", package="hBayesDM")  
  }
  ...
  modelName <- "bandit2arm_AvgRdeltaP"
