#' Two-Choice category-learning exemplar model with assimilation and contrast shifts
#' 
#' @description 
#' Exemplar model with representational shifts (GCM from Nosofsky 1986)
#' 
#' \strong{MODEL:}
#' Exemplar or GCM model with representational shifts
#' 
...
#' \describe{
#'  \item{\code{model}}{Character string with the name of the model (\code{"bandit2arm_deltaP"}).}
...
#'  \item{\code{"dim1"}}{Dimension 1 value).}
#'  \item{\code{"dim2"}}{Dimension 1 value).}
#' }
...
#' output <- bandit2arm_deltaP(data = "example", niter = 2000, nwarmup = 1000, nchain = 3, ncore = 3)
#' 
...
cat_exemplar_shiftR <- function(data          = "choose",
                             niter         = 3000, 
                             nwarmup       = 1000, 
                             nchain        = 4,
                             ncore         = 1, 
  ...
  if (modelRegressor) { # model regressors (for model-based neuroimaging, etc.)
    ...
    modelPath <- system.file("stan", "cat_exemplar_shiftR.stan", package="hBayesDM")  
  }

  # To see how long computations take
  startTime <- Sys.time() 
  
  # For using example data - change later for category learning data
  if (data=="example") {
  ...
  numPars <- 3
  POI     <- c("mu_wdim1", "mu_c", "mu_gam",
               "sigma",
               "wdim1", "c", "gam",
               "log_lik")
  
  modelName <- "cat_exemplar_shiftR"
  ...
  dim1  <- array(1, c(numSubjs, maxTrials) )
  dim2  <- array(1, c(numSubjs, maxTrials) )
  ...
  for (i in 1:numSubjs) {
    ...
    dim1[i, 1:useTrials] <- tmp$dim1
    dim2[i, 1:useTrials] <- tmp$dim2
  ...
  if (inits[1] != "random") {
    ...
    genInitList <- function() {
      list(
        mu_p   = c( qnorm(inits_fixed[1]), qnorm(inits_fixed[2] / 5), qnorm(inits_fixed[3]) ),
        sigma  = c(1.0, 1.0, 1.0),
        wdim1_pr   = rep(qnorm(inits_fixed[1]), numSubjs),
        c_pr = rep(qnorm(inits_fixed[2]/5), numSubjs),
        gam_pr   = rep(qnorm(inits_fixed[3]/5), numSubjs),
      )
    }
  } else {
    genInitList <- "random"
  ...
  for (i in 1:numSubjs) {
    if (indPars=="mean") {
      allIndPars[i, ] <- c( mean(wdim1[, i]), 
                            mean(c[, i]),
                            mean(gam[, i]))
    } else if (indPars=="median") {
      allIndPars[i, ] <- c( median(wdim1[, i]), 
                            median(c[, i]),
                            median(gam[, i]))
    } else if (indPars=="mode") {
      allIndPars[i, ] <- c( estimate_mode(wdim1[, i]),
                            estimate_mode(c[, i]),
                            estimate_mode(gam[, i]))
    }
  }
