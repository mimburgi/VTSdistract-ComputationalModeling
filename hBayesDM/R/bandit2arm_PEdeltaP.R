#' Two-Arm Bandit Task model with perseveration, learns value compared to last outcome
#' 
#' @description 
...
#' 
#' \strong{MODEL:}
#' Rescorla-Wagner (delta) model with decaying perseveration
#' 
...
#' \describe{
#'  \item{\code{model}}{Character string with the name of the model (\code{"bandit2arm_PEdeltaP"}).}
...
#' output <- bandit2arm_PEdeltaP(data = "example", niter = 2000, nwarmup = 1000, nchain = 3, ncore = 3)
#' 
...
bandit2arm_PEdeltaP <- function(data          = "choose",
                             niter         = 3000, 
                             nwarmup       = 1000, 
                             nchain        = 4,
                             ncore         = 1, 
  ...
  if (modelRegressor) { # model regressors (for model-based neuroimaging, etc.)
    ...
    modelPath <- system.file("stan", "bandit2arm_PEdeltaP.stan", package="hBayesDM")  
  }
  ...
  if (inits[1] != "random") {
    ...
    genInitList <- function() {
      list(
        ...
        P_pr   = rep(qnorm(inits_fixed[3]), numSubjs),
      )
    }
  } else {
    genInitList <- "random"
