#' Two-Arm Bandit Task Replacing Traces model with perseveration
#' 
#' @description 
...
#' \describe{
#'  \item{\code{model}}{Character string with the name of the model (\code{"bandit2arm_RTdeltaP"}).}
...
#' output <- bandit2arm_RTdelta(data = "example", niter = 2000, nwarmup = 1000, nchain = 3, ncore = 3)
#' 
...
bandit2arm_RTdeltaP <- function(data          = "choose",
                             niter         = 3000, 
                             nwarmup       = 1000, 
                             nchain        = 4,
                             ncore         = 1, 
  ...
  if (modelRegressor) { # model regressors (for model-based neuroimaging, etc.)
    ...
    modelPath <- system.file("stan", "bandit2arm_RTdeltaP.stan", package="hBayesDM")  
  }
