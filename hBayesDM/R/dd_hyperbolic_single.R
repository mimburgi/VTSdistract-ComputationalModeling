...
#' \describe{
#'  \item{\code{model}}{Character string with the name of the model ("dd_hyperbolic_single").}
...
dd_hyperbolic_single <- function(data     = NULL,
  ...
  if (modelRegressor) { # model regressors (for model-based neuroimaging, etc.)
    ...
    modelPath <- system.file("stan", "dd_hyperbolic_single.stan", package="hBayesDM")
  }
  ...
  if (indPars=="mean") {
    ...
    allIndPars <- c( median(k), 
                     median(logK),
                     median(beta) )
  } else if (indPars=="mode") {
    allIndPars <- c( estimate_mode(k),
                     estimate_mode(logK),
                     estimate_mode(beta) )
  }
