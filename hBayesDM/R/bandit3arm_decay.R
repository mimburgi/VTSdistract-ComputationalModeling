#' Three-Arm Bandit Task decay rule
#' 
#' @description 
...
#' \describe{
#'  \item{\code{model}}{Character string with the name of the model (\code{"bandit3arm_decay"}).}
...
#' output <- bandit3arm_decay(data = "example", niter = 2000, nwarmup = 1000, nchain = 3, ncore = 3)
#' 
...
bandit3arm_decay <- function(data          = "choose",
                             niter         = 3000, 
                             nwarmup       = 1000, 
                             nchain        = 4,
                             ncore         = 1, 
  ...
  if (modelRegressor) { # model regressors (for model-based neuroimaging, etc.)
    ...
    modelPath <- system.file("stan", "bandit3arm_decay.stan", package="hBayesDM")  
  }
  ...
  if (data=="example") {
    data <- system.file("extdata", "bandit3arm_exampleData.txt", package = "hBayesDM")
  } else if (data == "choose") {
    data <- file.choose()
  }
