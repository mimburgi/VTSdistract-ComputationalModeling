...
#' 
#' \strong{MODEL:}
#' Rescorla-Wagner (delta) model with Eligibility traces (Sutton & Barto 1998) 
#' 
...
#' \describe{
#'  \item{\code{model}}{Character string with the name of the model (\code{"bandit2arm_ETdelta"}).}
...
bandit2arm_ETdelta <- function(data          = "choose",
                             niter         = 3000, 
                             nwarmup       = 1000, 
                             nchain        = 4,
                             ncore         = 1, 
  ...
  if (modelRegressor) { # model regressors (for model-based neuroimaging, etc.)
    ...
    modelPath <- system.file("stan", "bandit2arm_ETdelta.stan", package="hBayesDM")  
  }
  ...
  numPars <- 3
  POI     <- c("mu_A", "mu_tau", "mu_gam", 
               "sigma",
               "A", "tau", "gam",
               "log_lik")
  
  modelName <- "bandit2arm_ETdelta"
