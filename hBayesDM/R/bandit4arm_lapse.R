...
#' }
#' 
bandit4arm_lapse <- function(data          = "choose",
                             niter         = 4000, 
                             nwarmup       = 2000,
                             nchain        = 4,
                             ncore         = 1, 
  ...
  if (modelRegressor) { # model regressors (for model-based neuroimaging, etc.)
    ...
    modelPath <- system.file("stan", "bandit4arm_lapse.stan", package="hBayesDM")  
  }
  ...
  numPars <- 5
  POI     <- c("mu_Arew", "mu_Apun", "mu_R", "mu_P", "mu_xi",
               "sigma", 
               "Arew", "Apun", "R", "P", "xi",
               "log_lik")
  
  modelName <- "bandit4arm_lapse"
  ...
  if (inits[1] != "random") {
    if (inits[1] == "fixed") {
      inits_fixed = c( 0.1, 0.1, 1.0, 1.0, 0.1 )
    } else {
    ...
    genInitList <- function() {
      list(
        ...
        mu_p = c( qnorm(inits_fixed[1]), qnorm(inits_fixed[2]), qnorm(inits_fixed[3]/30), qnorm(inits_fixed[4]/30), qnorm(inits_fixed[5]) ),
        ...
        xi_pr      = rep( qnorm(inits_fixed[5]), numSubjs) 
      )
    }
  } else {
    genInitList <- "random"
